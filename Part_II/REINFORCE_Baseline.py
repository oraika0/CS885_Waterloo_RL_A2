...
parser.add_argument('--policy_train_iter', type=int, default=1) 
...
POLICY_TRAIN_ITERS = args.policy_train_iter  # Training epochs
...
def train(S,A,returns):
    # print(f"Training shape {S.shape}, {A.shape}, {returns.shape}")
    ###############################
    # YOUR CODE HERE:
...
for epi in pbar:
    ...
    pbar.set_description("R25(%g, mean over 25 episodes)" % (last25Rs[-1]))
...
# plt.plot(range(N), valueLosses, 'r')
plt.xlabel('Episode')
plt.ylabel('Reward (averaged over last 25 episodes)')
plt.title("REINFORCE with Baseline, mode: " + args.mode)
# plt.savefig("Assiment2/Part_II/a2_part2_starter/images/reinforce_baseline-"+args.mode+".png")
plt.savefig("images/reinforce_baseline-"+args.mode+ "-policy_iter="+str(args.policy_train_iter)+".png")
